...
chrome.storage.local.get(['server', 'nick'], (res)=>{
    ...
    function syncChannels(){
        urlDict = {};
        tabUrls.forEach((url)=>{
            urlDict[sha256(url)] = url;
        });
        if(connected){
            socket.send(JSON.stringify({type: 'CHLIST', message: tabUrls.map(url => sha256(url))}))
        }
    }
    ...
    socket.onmessage = function(msg) {
        ...
        chrome.tabs.query({}, function(tabs) {
            let mappedMsg = JSON.parse(msg.data);
            mappedMsg.channel = urlDict[mappedMsg.channel];
            for(tab_id in tabs){
                chrome.tabs.sendMessage(tabs[tab_id].id, mappedMsg);
            }
        });
    };
    ...
    chrome.runtime.onMessage.addListener(
        function(request, sender, sendResponse) {
            socket.send(JSON.stringify({type: request.type, channel: sha256(request.channel), message: `${res.nick}: ${request.message}`}));
        }
    );
